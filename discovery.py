...
Librerias externas:
    ...
    - zeroconf: Librería externa para la implementación del protocolo mDNS en Python, utilizada para descubrir servicios en la red local.
...
def buscar_mdns():
    """
    Descubre el ESP32-WROOM-32U del subsistema hardware anunciado en la red local mediante el protocolo mDNS.

    A partir de la creación de un objeto ServiceBrowser de la librería zeroconf y la clase Oyente, busca un servicio mDNS en la red local que coincida con el definido
    ...
    Return:
        ...
        es la dirección IP del ESP32-WROOM-32U y PUERTO es el puerto del servicio mDNS.
    ...
    class Oyente:
        """
        Representa el oyente necesario para la búsqueda de servicios en la red local mediante protocolo mDNS de Zeroconf. Llama automáticamente a la
        función callback add_service tras encontrar el dispositivo buscado, actualizando la IP y puerto en el diccionario anteriormente creado. En caso de no encontrar
        el dispositivo y superar el tiempo límite establecido por config.TIMEOUT_SEGUNDOS_MDNS, cancela la búsqueda de servicios, detiene el hilo interno y
        cierra el socket de red empleado devolviendo None.
    ...
    # ha sido actualizado con una IP). Tras 5 segundos finaliza la búsqueda mDNS.
